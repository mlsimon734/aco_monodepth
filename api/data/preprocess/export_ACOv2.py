...
import src.devkits.acov2 as acov2
...
vid_dir = PATHS['acov2']/'videos'
colmap_dir = PATHS['acov2']/'colmap'
...
def save_settings(**kwargs):
    io.write_yaml(PATHS['acov2']/'splits'/'config.yaml', kwargs)


def export_scene(args):
    vid_file, cat = args
    seq = vid_file.stem
    seq_dir = PATHS['acov2']/seq

    # Export frames and decimate
    acov2.extract_frames(
    ...
    acov2.add_frames_to_split(seq_dir, cat, seq, 'all', p_train=p_train, skip=val_skip)
...
def main(args):
    ...
    cats = acov2.load_categories(subcats=False)
    video_files = io.get_files(vid_dir)
    assert len(cats) == len(video_files), f'Non-matching ACOv2 videos {len(video_files)} and labelled categories {len(cats)}'
