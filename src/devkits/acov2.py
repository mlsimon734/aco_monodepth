...
# CONSTANTS
# -----------------------------------------------------------------------------
Item = namedtuple('ACOv2Item', 'seq stem')
# -----------------------------------------------------------------------------
...
def get_url_file() -> Path:
    ...
    return PATHS['acov2']/'splits'/f'urls.txt'


def get_category_file() -> Path:
    """Get filename containing list of video URLs."""
    return PATHS['acov2']/'splits'/f'categories.txt'


def get_vid_files() -> list[Path]:
    """Get list of video filenames."""
    return sorted(f for f in (PATHS['acov2']/'videos').iterdir() if f.suffix == '.mp4')


def get_split_file(mode: str, split: str) -> Path:
    """Get the split filename for the specified `mode`."""
    file = PATHS['acov2']/'splits'/f'{split}'/f'{mode}_files.txt'
    return file


def get_seqs() -> tuple[str]:
    """Get tuple of sequences names in dataset."""
    dirs = io.get_dirs(PATHS['acov2'], key=lambda d: d.stem not in {'splits', 'videos', 'colmap'})
...
def get_intrinsics_file(seq: str) -> Path:
    ...
    return PATHS['acov2']/seq/f'intrinsics.txt'


def get_img_file(seq:str, stem: ty.U[str, int]) -> Path:
    """Get image filename based on the sequence and item number."""
    # return PATHS['acov2']/seq/f'{int(stem):010}.jpg'
    return PATHS['acov2']/seq/f'{int(stem)}.jpg'
# -----------------------------------------------------------------------------
...
def load_categories(subcats: bool = True) -> list[str]:
    """Load list of categories per ACOv2 scenes."""
...
# def extract_frames2(frame_dir: Path,
#                    save_dir: ty.N[Path] = None,
#                    n_keep: ty.N[int] = None,
#                    per_interval: ty.N[int] = None,
#                    overwrite: bool = False) -> None:
#     """Create softlinks for frames at specified intervals.

#     :param frame_dir: (Path) Directory containing the extracted frames.
#     :param save_dir: (None|Path) Export directory for softlinks. (Default is same as frame directory)
#     :param n_keep: (None|int) Number of frames to keep. (Default is all)
#     :param per_interval: (None|int) Interval over which to keep `n_keep` frames. (Default is all)
#     :param overwrite: (bool) If `True`, overwrite existing softlinks.
#     :return:
#     """
#     frame_files = io.get_files(frame_dir)
#     save_dir = save_dir or frame_dir.parent/frame_dir.stem
#     if not overwrite and io.has_contents(save_dir):
#         print(f'-> Skipping frames in directory "{frame_dir}"...')
#         return

#     io.mkdirs(save_dir)

#     print(f'-> Creating softlinks in "{save_dir}"...')
#     num_frames = len(frame_files)
    
#     if n_keep is None or per_interval is None:
#         # Keep all frames
#         for i, frame_file in enumerate(frame_files):
#             target_path = save_dir / f'{i+1:010d}.png'
#             os.symlink(frame_file, target_path)
#     else:
#         # Keep frames at specified intervals
#         indices = _non_uniform_decimate(range(num_frames), n_keep, per_interval)
...
def add_frames_to_split(path: Path, *splits: str, p_train: float = 0.9, skip: int = 100) -> None:
    """Add ACOv2 video frames to the desired split(s)."""
    seq = path.stem

    fs = [f.stem for f in io.get_files(path, key=lambda f: f.suffix == '.jpg')]
...
def estimate_intrinsics(path: Path,
    ...
    try:
        ...
        # assert len(cam) == 1
        if not len(cam) == 1:
            print(f'-> WARNING: Unexpected number of cameras {len(cam)} for seq "{seq}"')

        cam = io.lmap(float, cam[0].split()[2:])  # As [CAM_IDX, CAM_MODEL, *CAM_PARAMS]
        # assert cam[:2] == [1280, 720]
        # assert cam[:2] == [426, 240] or cam[:2] == [426, 238]
        if not (cam[:2] == [426, 240] or cam[:2] == [426, 238]):
            print(f'-> WARNING: Unexpected camera size {cam[:2]} for seq "{seq}"')
