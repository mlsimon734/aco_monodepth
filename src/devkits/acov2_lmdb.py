...
def get_split_file(mode: str, split: str) -> Path:
    ...
    file = PATHS['acov2_lmdb']/'splits'/f'{split}'/f'{mode}_files.txt'
    return file


def get_category_file() -> Path:
    """Get filename containing list of video URLs."""
    return PATHS['acov2_lmdb']/'splits'/f'categories.txt'


def get_seqs() -> tuple[str]:
    """Get tuple of sequences names in dataset."""
    dirs = io.get_dirs(PATHS['acov2_lmdb'], key=lambda d: d.stem not in {'splits', 'videos', 'colmap'})
